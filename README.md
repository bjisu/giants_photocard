# 롯데자이언츠 포토카드 컬렉션 (모바일 웹)

NFC 굿즈를 태그하면 열리는 세로 모바일 웹 콘텐츠입니다.
`스플래시 → 비닐 뜯기(연타) → 뒷면 카드 3장 → 1장 선택·뒤집기·희귀도 연출 → 결과 → 저장/공유/팬톡 자랑하기` 흐름으로 동작하며, **컬렉션은 하루에 한 번만** 가능합니다.
...
- KST(Asia/Seoul) 자정 기준으로 리셋됩니다. 이미 뽑은 날 재접속하면 **오늘의 카드**가 다시 표시되고, 저장/공유/팬톡 자랑하기는 계속 가능합니다. 하단에 다음 컬렉션까지 남은 시간이 카운트다운됩니다.
